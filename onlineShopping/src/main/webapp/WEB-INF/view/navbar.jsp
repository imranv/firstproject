...
<ul class="nav navbar-nav">
      <li class="active"><a style="color:#F5F5DC" href="home">HOME</a></li>
       <li class="dropdown">
      <a class="dropdown-toggle" data-toggle="dropdown" href=${category.name}>Categories<span class="caret"></span> </a>
      <ul class="dropdown-menu">
       <c:forEach items="${categoryList}" var="category">
        <li><a style="color:#000000" class="dropdown-toggle" data-toggle="dropdown" href="${category.name}"> ${category.name} 
        <ul class="dropdown-menu">
      <c:forEach items="${category.products}" var="product">
      <li><a style="color:#000000" class="dropdown dropdown-menu-right" href="productget-${product.id}"> ${product.name} </a>
    </li>
    </c:forEach>
    </ul>
    </a>
...
<form class="navbar-form navbar-left">
    ...
    <ul class="nav navbar-nav navbar-right">
       ...
       <c:if test="${pageContext.request.userPrincipal.name != null}">
       		<c:if test="${pageContext.request.userPrincipal.name != 'immu'}">
